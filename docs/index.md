...
| **Tools** | Built-in tool implementations (read, edit, write, shell) | [tools.md](tools.md) |
...
- **Everything is a process.** The agent loop, session store, and tool execution are all OTP servers (`:gen_statem`/`GenServer`) or supervised tasks. The BEAM scheduler handles concurrency.
- **Per-session isolation.** Each session owns its entire process tree. Terminating a session cleanly stops all its tools and streaming connections.
