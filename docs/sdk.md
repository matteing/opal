...
The CLI includes a TypeScript SDK (`packages/cli/src/sdk/`) that provides a typed client for the JSON-RPC protocol. It can be used independently of the Ink UI.
...
`packages/cli/src/sdk/protocol.ts` is **auto-generated** from `Opal.RPC.Protocol.spec()` via:

```bash
cd packages/core && mix run ../../scripts/codegen_ts.exs
```
...
To verify types are in sync: `cd packages/core && mix run ../../scripts/codegen_ts.exs --check`

## Source

- `packages/cli/src/sdk/protocol.ts` — Auto-generated types (do not edit)
- `packages/cli/src/sdk/client.ts` — JSON-RPC transport
- `packages/cli/src/sdk/session.ts` — Session class and event emitter
- `packages/cli/src/sdk/index.ts` — Public API exports
- `scripts/codegen_ts.exs` — Type generator
- `packages/core/lib/opal/rpc/protocol.ex` — Source of truth for all types
