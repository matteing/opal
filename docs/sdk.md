...
The CLI includes a TypeScript SDK (`cli/src/sdk/`) that provides a typed client for the JSON-RPC protocol. It can be used independently of the Ink UI.

## Architecture

```
protocol.ts  ← Auto-generated types from rpc_schema.json
    ↓
client.ts    ← JSON-RPC transport (stdio read/write, request/response correlation)
    ↓
session.ts   ← High-level Session class (prompt, sendPrompt, abort, events)
```

## Protocol Types

`cli/src/sdk/protocol.ts` is **auto-generated** from `opal/priv/rpc_schema.json` (which is produced from `Opal.RPC.Protocol.spec()`) via:

```bash
cd opal && mix run ../scripts/codegen_ts.exs
```
...
// Other operations
await session.sendPrompt("Skip that file"); // queued if busy
await session.abort();
...
The codegen script (`scripts/codegen_ts.exs`) reads `opal/priv/rpc_schema.json` and generates:
...
To verify types are in sync: `cd opal && mix run ../scripts/codegen_ts.exs --check`

## Source

- `cli/src/sdk/protocol.ts` — Auto-generated types (do not edit)
- `cli/src/sdk/client.ts` — JSON-RPC transport
- `cli/src/sdk/session.ts` — Session class and event emitter
- `cli/src/sdk/index.ts` — Public API exports
