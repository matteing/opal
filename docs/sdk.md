...
`cli/src/sdk/protocol.ts` is **auto-generated** from `Opal.RPC.Protocol.spec()` via:

```bash
cd core && mix run ../scripts/codegen_ts.exs
```
...
To verify types are in sync: `cd core && mix run ../scripts/codegen_ts.exs --check`
...
- `core/lib/opal/rpc/protocol.ex` — Source of truth for all types
