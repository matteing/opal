...
Pre-recorded SSE event sequences live in `test/support/fixtures/`:
...
| File | Purpose |
|------|---------|
| `test/test_helper.exs` | ExUnit config, tag exclusions |
| `test/support/fixture_helper.ex` | Fixture loading, `Req.Response.Async` simulation |
| `test/support/fixtures/*.json` | Pre-recorded SSE event sequences |
| `test/opal/live_test.exs` | Live API tests (general) |
| `test/opal/live_thinking_test.exs` | Live API tests for reasoning/thinking fixture recording |
| `test/opal/thinking_integration_test.exs` | Fixture-replay integration tests for thinking |
| `config/test.exs` | Logger level for tests |
