...
`Opal.Agent` is the runtime loop that drives prompt handling, model streaming, tool execution, retries, and completion. The loop is now implemented as an OTP `:gen_statem` (`core/lib/opal/agent/agent.ex`) with explicit lifecycle states, while helper modules under `core/lib/opal/agent/` keep stream parsing, tool orchestration, retries, and compaction concerns separated.
...
The agent runtime now follows a responsibility-first layout under `core/lib/opal/agent/`:
