defmodule Opal.Provider.RegistryTest do
  ...
  describe "list_provider/1" do
    ...
    test "returns empty list for unknown provider" do
      assert Opal.Provider.Registry.list_provider(:nonexistent_provider_xyz) == []
    end
  end

  describe "context_window/1" do
    test "returns context window for a known Copilot model" do
      model = %Opal.Provider.Model{provider: :copilot, id: "claude-opus-4.6"}
    ...
    test "returns context window for a direct provider model" do
      model = %Opal.Provider.Model{provider: :anthropic, id: "claude-sonnet-4-5"}
      ctx = Opal.Provider.Registry.context_window(model)
      assert ctx == 200_000
    end

    test "falls back to default for unknown model" do
      model = %Opal.Provider.Model{provider: :copilot, id: "nonexistent-model-xyz"}
  ...
  describe "resolve/1" do
    ...
    test "returns error for unknown model" do
      model = %Opal.Provider.Model{provider: :copilot, id: "nonexistent-xyz"}
