...
defmodule Codegen.TS do
  ...
  defp parse_result(%{"properties" => props} = _schema) when map_size(props) > 0 do
    props
    |> Enum.map(fn {name, field_schema} ->
      %{
        name: name,
  ...
  defp json_schema_to_type(%{"type" => "object", "properties" => props})
       when map_size(props) > 0 do
    fields =
      Map.new(props, fn {name, schema} ->
        {name, json_schema_to_type(schema)}
      end)
  ...
  # -- TypeScript generation (unchanged interface) --
